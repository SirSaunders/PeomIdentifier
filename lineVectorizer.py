...
#finds embeded vector index for given word
...
#Gerate a vector comparing a number of given lines 
...
#give corpus to vectorize line using Word2Vec word comparisions 
with open('combPoNo') as f:
    content = f.readlines()
    sentences = []
    even = 0
    for sentence in content: #clean lines a bit before comparision
        if sentence == '\n':
            if (len(sentences) != 0):
